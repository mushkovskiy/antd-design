...
### Доработки после первого ревью

1. **Исправить тестовые данные для `aptrProjects`**  
   Поле `aptrProjects` должно быть **только** в объектах массива `aptrBudgetObjects` (потребности есть только у объектов).  
   Удалить это поле из всех остальных мест.  
   При необходимости поправить логику рендера строк.

2. **BudgetProgressBar на 2 колонки**  
   Разместить `BudgetProgressBar` так, чтобы он занимал **две колонки** Стоимость и Бюджет.  
   Использовать `onCell` с свойством `colSpan={2}`.

3. **Добавить комментарии к сложным вычислениям**  
   Добавить **поясняющие комментарии** ко всем функциям с непростой логикой вычислений.  
   Комментарии должны объяснять: входные данные → алгоритм → результат.

4. **Убрать границы между подколонками**  
    Убрать **границы** между подколонками:  
    **Потребность | Стоимость | Бюджет | Отклонение**
   Текущий css селектор не убирает эти границы
