...
1. В тестовых данных есть ошибка, поле "aptrProjects" будет только в объектах массива "aptrBudgetObjects" так как потребности могут быть только у объектов, нужно убрать это поле отовсюду кроме объетков массива "aptrBudgetObjects". Поправь логику рендера строк если нужно.
2. BudgetProgressBar нужно разместить так чтобы он занимал две колонки. Используй onCell, у него есть свойство colSpan
3. Добавь поясняющие комментарии к функциям которые делают сложные вычисления, чтобы было понятно как они работают.
4. Осталсиь гранциы между подколонками **Потребность**, **Стоимость**, **Бюджет**, **Отклонение**
